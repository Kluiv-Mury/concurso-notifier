...
from db import (
    ...
    adicionar_concurso_enviado, notificacoes_ativas
)
...
async def buscar_e_enviar_concursos(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in usuarios:
        ...
        if not notificacoes_ativas(user_id):
            continue
        ...
        for c in concursos:
            if concurso_ja_enviado(user_id, c['id']):
                continue
            ...
            msg = (
                f"🔔 <b>{c['titulo']}</b>\n"
                f"💵 {c['salario_max']} | 🏢 {c['vagas']} vagas\n"
                f"📅 Até: {c['inscricoes_ate']}\n"
                f"🔗 {c['link']}"
            )
